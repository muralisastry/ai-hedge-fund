              ...
              for (const eventText of events) {
                ...
                try {
                  ...
                  if (eventTypeMatch && dataMatch) {
                    ...
                    console.log(
                      `Parsed backtest ${eventType} event:`,
                      eventData,
                    );
                    ...
                    switch (eventType) {
                      ...
                      case "progress":
                        ...
                        if (eventData.agent && eventData.agent !== "backtest") {
                          ...
                          const baseAgentKey = eventData.agent.replace(
                            "_agent",
                            "",
                          );
                          ...
                          const agentIds = params.graph_nodes.map(
                            (node) => node.id,
                          );
                          const uniqueNodeId =
                            agentIds.find(
                              (id) => extractBaseAgentKey(id) === baseAgentKey,
                            ) || baseAgentKey;
                        ...
                        // Handle backtest-specific progress updates
                          ...
                          if (eventData.analysis) {
                            try {
                              const backtestResultData = JSON.parse(
                                eventData.analysis,
                              );
                              ...
                              console.error(
                                "Error parsing backtest result data:",
                                error,
                              );
                            }
                          }
                      ...
                      case "complete":
                        ...
                        if (eventData.data) {
                          const backtestResults = {
                            decisions: {
                              backtest: { type: "backtest_complete" },
                            },
                            analyst_signals: {},
                            performance_metrics:
                        ...
                        if (flowId) {
                          ...
                          setTimeout(() => {
                            const currentConnection =
                              flowConnectionManager.getConnection(flowId);
                      ...
                      case "error":
                        ...
                        if (flowId) {
                          flowConnectionManager.setConnection(flowId, {
                            state: "error",
                            error:
                              eventData.message || "Unknown error occurred",
                            abortController: null,
                          });
                        }
                        break;
                  ...
                  console.error(
                    "Error parsing backtest SSE event:",
                    err,
                    "Raw event:",
                    eventText,
                  );
